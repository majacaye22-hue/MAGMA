    ...
    <div className="flex items-center gap-3" style={{ marginBottom: "16px" }}>
      <Link href={`/profile/${username}`}>
        <div
          ...
          style={{ width: "32px", height: "32px", borderRadius: "2px", backgroundColor: "#D85A30", color: "#0c0c0b", fontFamily: syne }}
        >
          {initials(post.profiles)}
        </div>
      </Link>
      <div className="flex flex-col gap-0.5">
        <Link href={`/profile/${username}`} className="hover:underline" style={{ fontSize: "12px", color: "#888780", fontFamily: mono }}>
          @{username}
        </Link>
        <span style={{ fontSize: "11px", color: "#444441", fontFamily: mono }}>
          {relativeTime(post.created_at)}
          {display_name && <span style={{ marginLeft: "8px", color: "#2a2a28" }}>·</span>}
          {display_name && <span style={{ marginLeft: "8px" }}>{display_name}</span>}
        </span>
      </div>
    </div>
  );
...
function EscritoView({ post, comments, currentUserId, currentProfile }: {
  ...
  return (
    <div className="mx-auto px-6 pb-24" style={{ maxWidth: "680px" }}>
      ...
      <style>{`
        .escrito-body { font-family: var(--font-space-mono), monospace; font-size: 15px; color: #888780; line-height: 1.9; margin-bottom: 40px; word-break: break-word; }
        .escrito-body p { margin: 0 0 16px; }
        .escrito-body p:last-child { margin-bottom: 0; }
        .escrito-body h2 { font-family: var(--font-syne), sans-serif; font-size: 22px; font-weight: 800; color: #e8e4dc; margin: 32px 0 12px; line-height: 1.2; letter-spacing: -0.01em; }
        .escrito-body h3 { font-family: var(--font-syne), sans-serif; font-size: 17px; font-weight: 700; color: #e8e4dc; margin: 24px 0 8px; line-height: 1.2; }
        .escrito-body strong { color: #c8c4bc; font-weight: 700; }
        .escrito-body em { color: #7a7672; }
        .escrito-body blockquote { border-left: 2px solid #7F77DD; padding-left: 16px; margin: 20px 0; color: #5F5E5A; font-style: italic; }
        .escrito-body ul { padding-left: 20px; list-style-type: disc; margin: 12px 0; color: #888780; }
        .escrito-body li { margin: 6px 0; }
        .escrito-body hr { border: none; border-top: 0.5px solid #2a2a28; margin: 32px 0; }
      `}</style>
