...
const FILTERS = [
  ...
  { label: "letras", tag: "manifiesto" },
] as const;

// ─── Post card ────────────────────────────────────────────────────────────────

function LetraCard({ post }: { post: EscritoPost }) {
...
export default async function LetrasPage({ searchParams }: PageProps) {
      ...
      <main className="max-w-3xl mx-auto px-6 pb-24">
        ...
        <div className="flex flex-wrap gap-2 pb-8">
          {FILTERS.map((f) => {
            ...
            const href = f.tag ? `/letras?tag=${f.tag}` : "/letras";
            return (
              <Link
                key={f.label}
                href={href}
        ...
        {posts.length === 0 ? (
          <div className="py-24 text-center text-xs tracking-widest uppercase" style={{ color: "#888780", fontFamily: mono }}>
            sin letras todavía — sé el primero en publicar
          </div>
        ) : (
          <div className="flex flex-col" style={{ gap: "2px" }}>
            {posts.map((post) => (
              <LetraCard key={post.id} post={post} />
            ))}
          </div>
        )}
      </main>
